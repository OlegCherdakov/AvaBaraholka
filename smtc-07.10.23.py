...
TEXT_CHECK_BTN = "Проверяем объявление перед публикацией"
...
TEXT_BEFORE_ADD_IMG = "Привет, житель планеты Ава-Петер. Я бот, и помогу тебе разместить объявление в АваБарахолке. Загрузи одну фотографию."
TEXT_AFTER_PULLICATION = "Объявление опубликовано. Для того, чтобы добавть новое объявление, напишите /start"


def start_keyboard():
   keyboard = types.ReplyKeyboardMarkup()
   send = types.InlineKeyboardButton(text='Создать объявление')
   keyboard.add(send)
   return keyboard
...
def text_handler(message):
    ...
    bot.send_message(
        message.chat.id,
        TEXT_CHECK_BTN, reply_markup=send_to_channel_keyboard()
    )
