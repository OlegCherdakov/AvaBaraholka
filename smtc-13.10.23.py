...
TEXT_FOR_ADD_IMG = 'Добавьте одну фотографию, остальные фотографии можно добавить в комментарии к объявлению после публикации. Подпись к фотографии вводить не нужно. Галочку "сжать фото" оставляем'
...
TEXT_AFTER_PUBLICATION = "Объявление опубликовано.\nДля того, чтобы добавть новое объявление, напишите или нажмите /start"
...
def text_handler(message):
    ...
    # проверяем, чтобы текст объявления не был слишком длинным
        ...
        bot.send_message(
            message.chat.id,
            "Укажите Ваше подразделение",
        )
        bot.register_next_step_handler(message, add_department)
    else:
        bot.send_message(
            message.chat.id,
            TEXT_IF_TOO_LONG,
