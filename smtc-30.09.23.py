...
# Вывод сообщения и кнопки для отправки объявления в Барахолку
...
def main(message):
    bot.send_message(
        message.chat.id,
        TEXT_PUBLICATE_BTN, reply_markup=start_keyboard()
    )
...
Конец вывод сообщения и кнопки для отправки объявления в Барахолку
###################################################################################
...
def send_to_users(message):
    ...
    bot.send_message(message.chat.id, TEXT_AFTER_PULLICATION)
